...
partial class MinimalWebView {
   ...
   public static MinimalWebView Init(MinimalWindow parentWindow, ILogger? logger) {
      MinimalWebView webView = new(logger);
      webView.initController(parentWindow.Handle);
      return webView;
   }

   
   private static async Task initControllerAsync(HWND hwnd, 
                                                 Action<CoreWebView2Controller> onInitialized,
                                                 Action? handleNavigationCompleted,
                                                 HandleMessageFromWebViewDelegate handleMessageFromWebView,
                                                 ILogger? logger) {
      CoreWebView2Controller webViewController = await initControllerActualAsync();
      onInitialized(webViewController);
      ...
      async Task<CoreWebView2Controller> initControllerActualAsync() {
         ...
         setupWebView(controller, hwnd, handleNavigationCompleted, handleMessageFromWebView);
         // navigateWebView(controller);
         return controller;
      }

      static void setupWebView(CoreWebView2Controller controller, HWND hwnd,
                               Action? handleNavigationCompleted,
                               HandleMessageFromWebViewDelegate handleMessageFromWebView) {
         controller.DefaultBackgroundColor          =  Color.Transparent; // avoids flash of white when page first renders
         controller.CoreWebView2.NavigationCompleted += (_, _) => handleNavigationCompleted?.Invoke();
   ...
   private static void navigateWebView(CoreWebView2Controller controller) {
   ...
   private static async Task<CoreWebView2Controller> createCoreWebView2Async(HWND hwnd, ILogger? logger) {
      try {
         logger?.LogDebug("Initializing WebView2"); // TODO: improve
